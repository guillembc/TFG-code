...
```{r}
#data <- read_excel("C:/Users/pc/Desktop/sp500.xlsx")
#attach(data)
```
...
sp500 <- as.matrix(read.table('../TFG/S&P500_log_returns.csv', header=TRUE, sep=' '))
```
...
Finally, we import from the FRED a time series dummy variable where it is equal to 1 if USA is in a recession and 0 otherwise. This will be used to compare GMV portfolio performance between when there is a recession versus when there is not.

```{r}
fred <- read.table('../TFG/JHDUSRGDPBR.csv', header=TRUE, sep=',')

fred$DATE <- as.Date(fred$DATE)
fred.xts <- xts(fred$JHDUSRGDPBR, order.by = fred$DATE)

recession <- na.locf(merge(fred.xts, foo=zoo(NA, order.by=seq(start(fred.xts), as.Date("2021-02-26"),
  "day",drop=F)))[, 1])
colnames(recession) <- "Recession"
```
...
```{r}
#1. EQUAL WEIGHT
demiguelR <- as.matrix(read.table('../TFG/Resultats/378/DeMiguel R.csv', header=TRUE, sep=' '))
demiguelW <- as.matrix(read.table('../TFG/Resultats/378/DeMiguel W.csv', header=TRUE, sep=' '))

#2. SAMPLE COVARIANCE
sampleR <- as.matrix(read.table('../TFG/Resultats/378/Sample_cov R.csv', header=TRUE, sep=' '))
sampleW <- as.matrix(read.table('../TFG/Resultats/378/Sample_cov W.csv', header=TRUE, sep=' '))

#3. LINEAR SHRINKAGE to IDENTITY MATRIX
wolfR <- as.matrix(read.table('../TFG/Resultats/378/LR sam_cov R.csv', header=TRUE, sep=' '))
wolfW <- as.matrix(read.table('../TFG/Resultats/378/LR sam_cov W.csv', header=TRUE, sep=' '))

#4. NON-LINEAR SHRINKAGE to IDENTITY MATRIX
sampleNlR <- as.matrix(read.table('../TFG/Resultats/378/NL sam_cov R.csv', header=TRUE, sep=' '))
sampleNlW <- as.matrix(read.table('../TFG/Resultats/378/NL sam_cov W.csv', header=TRUE, sep=' '))

#6. MARKET FACTOR with LINEAR SHRINKAGE IN RESIDUALS
marketR <- as.matrix(read.table('../TFG/Resultats/378/Market LR residuals R.csv', header=TRUE, sep=' '))
marketW <- as.matrix(read.table('../TFG/Resultats/378/Market LR residuals W.csv', header=TRUE, sep=' '))

#7. FAMA FRENCH 3 FACTORS with LINEAR SHRINKAGE IN RESIDUALS
fama3R <- as.matrix(read.table('../TFG/Resultats/378/Fama3 LR residuals R.csv', header=TRUE, sep=' '))
fama3W <- as.matrix(read.table('../TFG/Resultats/378/Fama3 LR residuals W.csv', header=TRUE, sep=' '))

#8.  FAMA FRENCH 5 FACTORS with LINEAR SHRINKAGE IN RESIDUALS
fama5R <- as.matrix(read.table('../TFG/Resultats/378/Fama5 LR residuals R.csv', header=TRUE, sep=' '))
fama5W <- as.matrix(read.table('../TFG/Resultats/378/Fama5 LR residuals W.csv', header=TRUE, sep=' '))

#9. LINEAR SHRINKAGE to CONSTANT CORRELATION (LEDOIT and WOLF)
con_wolfR <- as.matrix(read.table('../TFG/Resultats/378/Con_corr LW R.csv', header=TRUE, sep=' '))
con_wolfW <- as.matrix(read.table('../TFG/Resultats/378/Con_corr LW W.csv', header=TRUE, sep=' '))

#11. PCA with 1 PCs
pca1R <- as.matrix(read.table('../TFG/Resultats/378/PCA1 LR residuals R.csv', header=TRUE, sep=' '))
pca1W <- as.matrix(read.table('../TFG/Resultats/378/PCA1 LR residuals W.csv', header=TRUE, sep=' '))

#12. PCA with 3 PCs
pca3R <- as.matrix(read.table('../TFG/Resultats/378/PCA3 LR residuals R.csv', header=TRUE, sep=' '))
pca3W <- as.matrix(read.table('../TFG/Resultats/378/PCA3 LR residuals W.csv', header=TRUE, sep=' '))

#13. PCA with 5 PCs
pca5R <- as.matrix(read.table('../TFG/Resultats/378/PCA5 LR residuals R.csv', header=TRUE, sep=' '))
pca5W <- as.matrix(read.table('../TFG/Resultats/378/PCA5 LR residuals W.csv', header=TRUE, sep=' '))
```
...
write.table(demiguelW,'../TFG/Resultats/378/DeMiguel W.csv', sep=' ', row.names=TRUE, col.names=TRUE)
write.table(demiguelR,'../TFG/Resultats/378/DeMiguel R.csv', sep=' ', row.names=TRUE, col.names=TRUE)
```
...
write.table(sampleW,'../TFG/Resultats/378/Sample_cov W.csv', sep=' ', row.names=TRUE, col.names=TRUE)
write.table(sampleR,'../TFG/Resultats/378/Sample_cov R.csv', sep=' ', row.names=TRUE, col.names=TRUE)
```
...
write.table(wolfW,'../TFG/Resultats/378/LR sam_cov W.csv', sep=' ', row.names=TRUE, col.names=TRUE)
write.table(wolfR,'../TFG/Resultats/378/LR sam_cov R.csv', sep=' ', row.names=TRUE, col.names=TRUE)
```
...
write.table(sampleNlW,'../TFG/Resultats/378/NL sam_cov W.csv', sep=' ', row.names=TRUE, col.names=TRUE)
write.table(sampleNlR,'../TFG/Resultats/378/NL sam_cov R.csv', sep=' ', row.names=TRUE, col.names=TRUE)
```
...
write.table(marketW,'../TFG/Resultats/378/Market LR residuals W.csv', sep=' ', row.names=TRUE, col.names=TRUE)
write.table(marketR,'../TFG/Resultats/378/Market LR residuals R.csv', sep=' ', row.names=TRUE, col.names=TRUE)
```
...
write.table(fama3W,'../TFG/Resultats/378/Fama3 LR residuals W.csv', sep=' ', row.names=TRUE, col.names=TRUE)
write.table(fama3R,'../TFG/Resultats/378/Fama3 LR residuals R.csv', sep=' ', row.names=TRUE, col.names=TRUE)
```
...
write.table(fama5W,'../TFG/Resultats/378/Fama5 LR residuals W.csv', sep=' ', row.names=TRUE, col.names=TRUE)
write.table(fama5R,'../TFG/Resultats/378/Fama5 LR residuals R.csv', sep=' ', row.names=TRUE, col.names=TRUE)
```
...
write.table(con_wolfW,'../TFG/Resultats/378/Con_corr LW W.csv', sep=' ', row.names=TRUE, col.names=TRUE)
write.table(con_wolfR,'../TFG/Resultats/378/Con_corr LW R.csv', sep=' ', row.names=TRUE, col.names=TRUE)
```
...
write.table(pca1W,'../TFG/Resultats/378/PCA3 LR residuals W.csv', sep=' ', row.names=TRUE, col.names=TRUE)
write.table(pca1R,'../TFG/Resultats/378/PCA3 LR residuals R.csv', sep=' ', row.names=TRUE, col.names=TRUE)
```
...
write.table(pca3W,'../TFG/Resultats/378/PCA3 LR residuals W.csv', sep=' ', row.names=TRUE, col.names=TRUE)
write.table(pca3R,'../TFG/Resultats/378/PCA3 LR residuals R.csv', sep=' ', row.names=TRUE, col.names=TRUE)
```
...
write.table(pca5W,'../TFG/Resultats/378/PCA5 LR residuals W.csv', sep=' ', row.names=TRUE, col.names=TRUE)
write.table(pca5R,'../TFG/Resultats/378/PCA5 LR residuals R.csv', sep=' ', row.names=TRUE, col.names=TRUE)
```
...
At this point we divide portfolio returns in two groups: the first corresponds to a non-recession period (according to US GDP data) while the second corresponds to a recession period. Assuming that the market experiences much less volatility during a non-recession period, our purpose is to compare the portfolio's performance when there is turmoil in markets and where is not.

```{r}
data <- cbind(demiguelR, sampleR, wolfR, sampleNlR, marketR, fama3R, fama5R, con_wolfR, pca1R, pca3R, pca5R)
recession <- as.matrix(recession)
data2 <- merge(recession, data, by=0)
data2<- as.data.frame(data2)
rownames(data2) <- as.Date(as.character(data2$Row.names))
data2 <- data2[,2:ncol(data2)]

bull <- as.matrix(subset(data2, Recession==0))
bear <- as.matrix(subset(data2, Recession==1))
```

We compute the annualized volatility in percentage for each period: non-recession (bull) and recession(bear).

```{r}
bull_vol <- colSds(bull[,2:ncol(bull)]/100)*sqrt(252)*100
bear_vol <- colSds(bear[,2:ncol(bear)]/100)*sqrt(252)*100

perc_change <- 100*(bear_vol-bull_vol)/bull_vol
```

Summary of the results

```{r}
table3 <- cbind(bull_vol, bear_vol, perc_change)
rownames(table3) <- c("Equal Weight", "Sample Covariance", "Linear Shrinkage", "Non-linear shrinkage", "Market factor", "Fama-French (3)", "Fama-French(5)", "Constant Correlation","PCA1", "PCA 3", "PCA 5")
colnames(table3) <- c("Annualized Vol(%)", "Ann Vol(%) in recessions", "Percentage change(%)")
(round(table3, 3))
```
...
```{r}
EqualWeight <- as.data.frame(100*cumprod(1+demiguelR/100))
colnames(EqualWeight)<- "Equal Weight"
EqualWeight$dates <- dates[(a+1):T]

SampleCovariance<- as.data.frame(100*cumprod(1+sampleR/100))
colnames(SampleCovariance) <- "Sample Covariance"

LinearShrinkage <- as.data.frame(100*cumprod(1+wolfR/100))
colnames(LinearShrinkage) <- "Linear Shrinkage"                                    
                                    
MarketFactor<- as.data.frame(100*cumprod(1+marketR/100))
colnames(MarketFactor) <- "Market Factor + LS in res"

FF5<- as.data.frame(100*cumprod(1+fama5R/100))
colnames(FF5) <- "FF5 + LS in res"

PCA5<- as.data.frame(100*cumprod(1+pca5R/100))
colnames(PCA5) <- "PCA 5 + LS in res"

data3 <- as.data.frame(cbind(EqualWeight, SampleCovariance, LinearShrinkage, MarketFactor, FF5, PCA5))
data3 <- melt(data3, id="dates")

data3%>%
  ggplot(aes(x=dates, y=value, colour=variable))+
  geom_line()+
  ggtitle("PERFORMANCE OF EACH PORTFOLIO")+
  xlab("")+
  ylab("")+
  scale_y_continuous(trans = "log10")+
  scale_color_brewer(palette = "Dark2")+
  theme_bw()+
  theme(legend.title = element_blank(), legend.position = c(0.85,0.30))
  
```

```{r}
library(roll)
data4 <- cbind(demiguelR, sampleR, wolfR, marketR, fama5R, pca5R)/100

data4 <- as.data.frame(na.omit((roll_sd(data4, width=251))*sqrt(252)*100))
data4$dates <- dates[(a+251):T]

data4 <- melt(data4, id="dates")

data4%>%
  ggplot(aes(x=dates, y=value, colour=variable))+
  geom_line()+
  ggtitle("VOLATILITY OF EACH PORTFOLIO")+
  xlab("")+
  ylab("Annualized Volatility (%)")+
  theme_bw()+
  theme(legend.title = element_blank(), legend.position = c(0.7,0.7))+
  scale_colour_manual(labels=c("Equal Weight", "Sample Covariance", "Linear Shrinkage", "Market factor + LS in res", "FF5 + LS in res", "PCA 5 + LS in res"), values=c("#1B9E77", "#D95F02", "#7570B3", "#E7298A", "#66A61E", "#E6AB02"))
```

```{r}
data5 <- data4[which(data4$variable!="Equal.weight"),]
...
data5%>%
  ...
  ylab("Annualized Volatility (%)")+
  theme_bw()+
  theme(legend.title = element_blank(), legend.position = c(0.65,0.7))+
  scale_colour_manual(labels=c("Sample Covariance", "Linear Shrinkage", "Market factor + LS in res", "FF5 + LS in res", "PCA 5 + LS in res"), values=c("#D95F02", "#7570B3", "#E7298A", "#66A61E", "#E6AB02"))
```
